...
class TimeBlock(nn.Module):
    """
    Un bloc qui implémente la transformation linéaire en temps: (time*A + B)*data + C
    """
    def __init__(self, in_channels):
        super(TimeBlock, self).__init__()
        
        # Pour A: transformation qui sera multipliée par le temps
        self.A = nn.Linear(in_channels, in_channels, bias=False)
        
        # Pour B: transformation indépendante du temps
        self.B = nn.Linear(in_channels, in_channels, bias=False)
        
        # Pour C: biais additif
        self.C = nn.Parameter(torch.zeros(in_channels), requires_grad=True)
    ...
    def forward(self, x, time):
        """
        Applique la transformation (time*A + B)*x + C
        
        Args:
            x: Tenseur d'entrée [batch_size, ..., in_channels]
            time: Tenseur contenant les valeurs de temps [batch_size]
            
        Returns:
            Tenseur transformé de même forme que x
        """
        # Sauvegarde de la forme originale
        original_shape = x.shape
        batch_size = original_shape[0]
        
        # Reshape en [batch_size, -1, in_channels] pour appliquer la transformation
        x_flat = x.view(batch_size, -1, original_shape[-1])
        
        # Appliquer la transformation (time*A + B)*data + C pour chaque élément du batch
        outputs = []
        
        for b in range(batch_size):
            # Extraire les features pour cet élément du batch
            features = x_flat[b]  # [-1, in_channels]
            
            # Appliquer la transformation
            time_val = time[b]
            transformed = self.A(features) * time_val + self.B(features) + self.C
            
            outputs.append(transformed)
        
        # Rassembler les résultats et restaurer la forme d'origine
        output = torch.stack(outputs)  # [batch_size, -1, in_channels]
    ...
    def forward(self, x,time):
        time_transformation = self.create_time_tranformation(time)
        x= time_transformation(x)
        return x
