...
### Classes modules for model architectures   ###

class PositionalEmbedding(nn.Module):
    '''
    Positional Embedding for Vision Transformer (ViT).
    Supports both learnable and sinusoidal embeddings.
    '''
    def __init__(self,
                 num_patches,
                 embed_dim,
        ...
        Args:
            ...
            embedding_type (str): 'learnable' or 'sinusoidal'
    ...
    def forward(self, x):
        ...
        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, seq_len, embed_dim)
                             where seq_len = num_patches

        Returns:
            torch.Tensor: x + positional embeddings with dropout applied
        '''
        batch_size, seq_len, _ = x.shape

        # Dimension verification
        if seq_len != self.num_positions:
            raise ValueError(f"Sequence length {seq_len} does not match "
                           f"the number of positions {self.num_positions}")
    ...
    def interpolate_pos_embedding(self, new_num_patches):
        '''
        Interpolates positional embeddings for a new image size
        Useful for fine-tuning with different resolutions
        '''
        if self.embedding_type != 'learnable':
            raise NotImplementedError("Interpolation is only supported for learnable embeddings")
...
class PatchEmbedding(nn.Module):
    ...
    Divides the image into patches and projects them into embeddings.
    '''
...
class MultiHeadSelfAttention(nn.Module):
    """
    Multi-head self-attention module with variable number of heads.
    Implements the self-attention mechanism where the input is split into multiple heads,
    attention is computed in parallel for each head, and the results are concatenated.
    """
    ...
    def forward(self, x, mask=None):
        ...
        Args:
            ...
            mask: Optional tensor of shape [batch_size, seq_len, seq_len] or None
